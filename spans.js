...
define([], function(){
    ...
    function Compound(ranges) {
        this._ranges = ranges;
        // assert sorted?
    }

    Compound.prototype.min = function() {
        return this._ranges[0].min();
    };

    Compound.prototype.max = function() {
        return this._ranges[this._ranges.length - 1].max();
    };

    Compound.prototype.contains = function(pos) {
    ...
    Compound.prototype.isContiguous = function() {
        return this._ranges.length > 1;
    };

    Compound.prototype.ranges = function() {
        return this._ranges;
    };

    Compound.prototype._pushRanges = function(ranges) {
    ...
    function intersection(s0, s1) {
        ...
        while (i0 < l0 && i1 < l1) {
            var s0_bis = r0[i0], s1_bis = r1[i1];
            var lapMin = Math.max(s0_bis.min(), s1_bis.min());
            var lapMax = Math.min(s0_bis.max(), s1_bis.max());
            if (lapMax >= lapMin) {
                or.push(new Range(lapMin, lapMax));
            }
            if (s0_bis.max() > s1_bis.max()) {
                ++i1;
            } else {
                ++i0;
            }
